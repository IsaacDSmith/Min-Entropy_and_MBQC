"""
This script generates the comb D_{client}^{(m)} for |A| angles and m rounds for  
the minimal example given by the vertex, edge and output sets:
	
		G: 	V = {1, 2, 3}, E = {(1,2), (2,3), (1,3)}
		 	O = {2,3}  

There are two gflows for this example, g and g_prime (corresponding to g1 and g2 
in the text).
...
from guessing_probability import list_guessing_probability
		...
		for c2 in range(2):
			for c3 in range(2):
				...
				if (one_time_pads[0] + c1)%2 == 0:
					...
					alpha_two_prime = (
						((total_num_angles - angles[1])			# -alpha_{2}
						+ one_time_pads[1]*total_num_angles/2)	# + r_{2}pi
						%total_num_angles						# mod 2pi
						)
		...
		for c2 in range(2):
			for c3 in range(2):
				...
				if (one_time_pads[0] + c1)%2 == 0:
					...
					alpha_three_prime = (
						((total_num_angles - angles[2])			# -alpha_{3}
						+ one_time_pads[2]*total_num_angles/2)	# + r_{3}pi
						%total_num_angles						# mod 2pi
						)
