...
The following is adapted from cvxpy.atoms.affine.partial_trace.py as at 
14.10.2022. The aim is to be as close as possible to the original while 
implementing the partial trace over a 1D expression (to be interpreted as the
diagonal of a matrix).

The only changes to the original code exist in the _term() function, namely
by removing the operator 'a' from the code. In most cases, it has simply been
commented out, however the return of the original function _term is a@expr@b 
so in this case it has been deleted. The remainder of the code is verbatim from 
the original except for minor changes: the name of the function is now
list_partial_trace, there are formating changes to the comments.
"""
